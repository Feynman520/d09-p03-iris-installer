...
- **`--auto` 자가 시작(2.0.22)**: 자격이 있으면 서버가 `listen` 0.8초 뒤 스스로 `beginAuto()`(= `POST /api/auto` 와 같은 함수)를 부른다. 브라우저 탭은 진행을 보여 주는 용도일 뿐이며, 탭이 `POST /api/auto` 를 또 불러도 `202 running`. 끄기 = `IRIS_INSTALLER_AUTO_SELFSTART=0`(옵션 `autoSelfStart:false`, 시험용).
- **손 실행 업데이트 제안(2.0.22)**: 손으로 실행한 설치기가 v2 영수증(세팅·온라인 끝남, 영수증 판 < 이 zip 판) 위에 있으면 서버 시작 판정과 `POST /api/locate` 가 `step:"update"` 로 두고(`state.auto` 는 `--auto` 와 같은 자격 + `viaWizard:true`), locate 응답에 `update:{from,to}` 를 싣는다. 화면은 `card-update`(「업데이트 시작」→ `POST /api/auto` → `card-auto` 와 같은 진행). 같은 판·더 새 판이면 전처럼 `done`(완료 요약). 2026-09-19 실제 사용자 실측: "21 을 받아 실행해도 곧바로 설치 완료"의 처방.
